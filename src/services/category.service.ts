...
export class CategoryService {
  ...
  async update(id: string, category: Category) {
    ...
    return await this.categoryRepository.update(_category);
  }

  async delete(id: string) {
    if (await this.productRepository.getCountByCategory(id) > 0) {
      throw new ValidationError("Não é possível deletar a categoria pois existem produtos associados a ela.");
    }
